...
test('cart - remove item', async ({ page }) => {
  ...
  await page.locator('.cart-single-list .remove-item').first().click();
  const cartItem = await page.locator('.cart-single-list').first();
  await expect(cartItem).toBeHidden();
  await page.screenshot({ path: 'tests/cart/screenshots/remove-item.png' });
});