...
test('cart - add quantity', async ({ page }) => {
  ...
  const expectedTotal = (totalPrice * 24).toLocaleString('en-US', { style: 'currency', currency: 'USD' });
  await expect(page.locator('.cart-items .total .total-amount')).toHaveText(expectedTotal);
  await page.screenshot({ path: 'tests/cart/screenshots/add-quantity-item.png' });
});