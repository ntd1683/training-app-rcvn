...
test('Shop - Search with page and perpage', async ({ page }) => {
  ...
  const selectPerPage = page.locator('.product-grids .tab-content .row select.form-select');
  ...
  expect(afterUrl).toEqual(urlShop + "?page=2&per_page=5");

  await page.mouse.wheel(0, -10000);
  await page.waitForTimeout(500);
  await page.screenshot({ path: 'tests/shop/screenshots/search-with-page.png' });
});
