...
test('Shop - Delete Search', async ({ page }) => {
  ...
  await page.waitForSelector('.product-grids .row .product-sidebar .button .btn.w-100');
  await page.getByRole('button', { name: 'Xoá Tìm Kiếm', exact: true }).click();

  await page.waitForURL(url => url !== afterUrl);
  const afterUrlV1 = page.url();
  expect(afterUrlV1).toEqual(urlShop);

  await page.mouse.wheel(0, -10000);
  await page.waitForTimeout(500);
  await page.screenshot({ path: 'tests/shop/screenshots/delete-search.png' });
});
