...
test('order-detail - check product', async ({ page }) => {
  ...
  const product = page.locator('.order-detail.container-fluid .products-info a');
  await expect(product).toBeVisible();
  await product.click();

  await page.waitForURL(url => url.pathname.startsWith('/san-pham/'), { timeout: 3000 });
  await expect(page.locator('.item-details.section').first()).toBeVisible();

  await page.mouse.wheel(0, -1000);
  await page.waitForTimeout(500);
  await page.screenshot({ path: 'tests/order-detail/screenshots/check-product.png' });
});