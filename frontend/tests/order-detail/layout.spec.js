...
test('order-detail - layout', async ({ page }) => {
  ...
  await page.waitForURL(url => url.pathname.startsWith('/don-hang-chi-tiet/'), { timeout: 30000 });
  await expect(page.locator('.order-detail.container-fluid').first()).toBeVisible();
  await expect(page.locator('.order-detail.container-fluid .text-muted.order-code')).toContainText(orderCode);

  await page.mouse.wheel(0, -100);
  await page.waitForTimeout(500);
  await page.screenshot({ path: 'tests/order-detail/screenshots/layout.png' });
});