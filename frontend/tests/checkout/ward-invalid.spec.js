...
test('checkout - ward invalid', async ({ page }) => {
  ...
  const wardOptions = wardSelect.locator('option');
  const count = await wardOptions.count();
  expect(count).toEqual(1);

  await wardSelect.click();
  await page.waitForTimeout(100);
  await page.screenshot({ path: 'tests/checkout/screenshots/ward-invalid.png' });
});