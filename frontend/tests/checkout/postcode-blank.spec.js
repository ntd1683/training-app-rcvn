...
test('checkout - postcode blank', async ({ page }) => {
  ...
  await page.locator('input[name="postCode"]').fill('');

  await page.locator('.checkout-wrapper.section .price-table-btn button').click();
  await expect(page.getByText('Mã bưu điện không được bỏ trống')).toBeVisible();
  await page.mouse.wheel(0, 250);
  await page.waitForTimeout(500);
  await page.screenshot({ path: 'tests/checkout/screenshots/postcode-blank.png' });
});