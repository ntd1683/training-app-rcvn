...
test('checkout - cancel checkout', async ({ page }) => {
  ...
  await popup.waitForURL(/paypal\.com/, { timeout: 30000 });
  await popup.close();

  await page.waitForTimeout(1000);
  await expect(page.getByRole('alert')).toContainText('Thanh toán đã bị huỷ!');

  await page.waitForTimeout(500);
  await page.screenshot({ path: 'tests/checkout/screenshots/cancel-checkout.png' });
});