...
test('checkout - check btn checkout layout', async ({ page }) => {
  ...
  await expect(page.getByText('Số điện thoại không được bỏ trống')).toBeVisible();
  await expect(page.getByText('Tỉnh/Thành phố không được bỏ trống')).toBeVisible();
  await expect(page.getByText('Phường không được bỏ trống')).toBeVisible();
  await expect(page.getByText('Địa chỉ không được bỏ trống')).toBeVisible();
  await expect(page.getByText('Mã bưu chính không được bỏ trống')).toBeVisible();
  await expect(page.getByText('Mã bưu chính không được bỏ trống')).toBeVisible();

  await page.waitForTimeout(1000);
  await page.screenshot({ path: 'tests/checkout/screenshots/check-btn-checkout-layout.png' });
});